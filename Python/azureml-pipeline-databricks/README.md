...
| LOCATION                  | westus2                                                  |
| TERRAFORM_BACKEND_RG      | dataops-terraform                                        |
| TERRAFORM_BACKEND_STORAGE | [The name of the storage account you created]            |
...
Create a service connection to your ML workspace via the [Azure DevOps Azure ML task instructions](https://marketplace.visualstudio.com/items?itemName=ms-air-aiagility.vss-services-azureml) to be able to execute the Azure ML training pipeline. Name the connection `DataOpsML Azure DevOps Azure ML Workspace` (this name is used in the variable `WORKSPACE_SVC_CONNECTION` in [azure-pipelines.yml](azure-pipeline.yml).
...
1. **Continuous Integration:** triggered on code change to master branch on GitHub,
performs linting, unit testing and publishes a training pipeline.
1. **Trigger ML Training Pipeline**: invokes the Azure ML service to trigger the published training pipeline to train, evaluate, and register a model.
